...
NECK_ANGLE_THRESH  = float(os.getenv("NECK_ANGLE_THRESH", "155")) # degrees; below = slouching
...
def classify_posture(landmarks, w: int, h: int) -> tuple[str, float]:
    l_ear, l_ear_v = get_landmark_coords(landmarks.landmark[mp_pose.PoseLandmark.LEFT_EAR], w, h)
    r_ear, r_ear_v = get_landmark_coords(landmarks.landmark[mp_pose.PoseLandmark.RIGHT_EAR], w, h)
    ear, ear_v = (l_ear, l_ear_v) if l_ear_v > r_ear_v else (r_ear, r_ear_v)

    l_sh, l_sh_v = get_landmark_coords(landmarks.landmark[mp_pose.PoseLandmark.LEFT_SHOULDER], w, h)
    r_sh, r_sh_v = get_landmark_coords(landmarks.landmark[mp_pose.PoseLandmark.RIGHT_SHOULDER], w, h)
    sh, sh_v = (l_sh, l_sh_v) if l_sh_v > r_sh_v else (r_sh, r_sh_v)

    l_hip, l_hip_v = get_landmark_coords(landmarks.landmark[mp_pose.PoseLandmark.LEFT_HIP], w, h)
    r_hip, r_hip_v = get_landmark_coords(landmarks.landmark[mp_pose.PoseLandmark.RIGHT_HIP], w, h)
    hip, hip_v = (l_hip, l_hip_v) if l_hip_v > r_hip_v else (r_hip, r_hip_v)

    if ear_v < 0.3 or sh_v < 0.3 or hip_v < 0.3:
        return "UNKNOWN", 0.0
...
def classify_activity(landmarks, w: int, h: int, prev_wrist_pos: dict) -> str:
    l_wrist, l_wrist_v = get_landmark_coords(landmarks.landmark[mp_pose.PoseLandmark.LEFT_WRIST], w, h)
    r_wrist, r_wrist_v = get_landmark_coords(landmarks.landmark[mp_pose.PoseLandmark.RIGHT_WRIST], w, h)
...
def draw_overlay(frame: np.ndarray, landmarks, posture: str, activity: str, analytics: dict) -> np.ndarray:
    ...
    cv2.putText(annotated, f"Posture : {posture}",    (10, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.75, color, 2)
    cv2.putText(annotated, f"Activity: {activity}",  (10, 56), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 255, 255), 2)
    cv2.putText(annotated, f"Good%   : {analytics['good_posture_percent']:.1f}%", (10, 84), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 200, 0), 2)
    cv2.putText(annotated, f"Tracked : {analytics['total_tracked_seconds']:.0f}s", (10, 112), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (200, 200, 200), 2)
    return annotated

def main():
    log.info("Opening video source: %s", WEBCAM_INDEX)
    # If WEBCAM_INDEX is an integer it's a device, if it's a string path it's a file
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while True:
            ...
            if results.pose_landmarks:
                presence   = "ACTIVE"
                posture_state, neck_angle = classify_posture(results.pose_landmarks, w, h)
                activity   = classify_activity(results.pose_landmarks, w, h, prev_wrist_pos)
            ...
            good_pct = (total_good_seconds / total_tracked_seconds * 100) if total_tracked_seconds > 0 else 0.0
            ...
            if os.getenv("DISPLAY"):
                annotated = draw_overlay(frame, results.pose_landmarks, posture_state, activity, analytics)
